...
func printHelpForPath(path []string) bool {
	...
	case "issue cleanup":
		fmt.Println("Usage: az ticket cleanup [--project <project-id>] [--id <issue-id> ...|--ids a,b] [--status <status> ...] [--query text] [--updated-before date] [--limit N] [--action closed|cancelled] [--dry-run] [--per-issue-timeout duration] [--json]")
	case "issue delete":
...
const (
	...
	issueGetUsage                 = "Usage: az ticket get [--project <project-id>] [--id <issue-id>] [--json] [--with-notes] [<issue-id>]"
	issueClaimUsage               = "Usage: az ticket claim [--project <project-id>] [--id <issue-id>] [--owner <owner-id>] [--kind human|agent|orchestrator] [--ttl 2h] [--force] [--json] [<issue-id>]"
	issueTakeoverUsage            = "Usage: az ticket takeover [--project <project-id>] [--id <issue-id>] [--owner <owner-id>] [--kind human|agent|orchestrator] [--ttl 2h] [--json] [<issue-id>]"
	issueReleaseUsage             = "Usage: az ticket release [--project <project-id>] [--id <issue-id>] [--owner <owner-id>] [--force] [--json] [<issue-id>]"
	issueEventsUsage              = "Usage: az ticket events [--project <project-id>] [--id <issue-id>] [--json] [--jq-help] [--type <event-type> ...] [--types a,b] [--limit N] [<issue-id>]"
	issueRecordUsage              = "Usage: az ticket record [--project <project-id>] [--id <issue-id>] [--type <event-type>] [--summary <text>] [--body <text>] [--data <json-object>] [--follow-up <issue-id> ...] [--json] [<issue-id>]"
	issueContextRiskUsage         = "Usage: az ticket context-risk [--project <project-id>] [--id <issue-id>] [--since 14d] [--summary|--full] [--json] [<issue-id>]"
	issueGetManyUsage             = "Usage: az ticket get-many [--project <project-id>] --id <issue-id> [--id <issue-id> ...] [--ids a,b,c] [--json] [--with-notes]"
	issueCheckUsage               = "Usage: az ticket check [--project <project-id>] [--id <issue-id>] [--json] [<issue-id>]"
	issueDoctorUsage              = "Usage: az ticket doctor [--project <project-id>] [--id <issue-id>] [--checkpoint-wal] [--truncate-wal] [--json] [<issue-id>]"
	issueUpdateUsage              = "Usage: az ticket update [--project <project-id>] [--id <issue-id>] [--json] [<issue-id>] [--title text] [--description text] [--notes text] [--append-notes text] [--status backlog|open|in_progress|in_review|closed|cancelled] [--cascade-children] [--force-worktree] [--type task|bug|feature|epic|chore|investigation] [--priority P0|P1|P2|P3|P4] [--update-impl <impl> ...]"
	issueCloseUsage               = "Usage: az ticket close [--project <project-id>] [--id <issue-id>|-i <issue-id>] [--json] [--force-worktree] [--close-clean-children] [<issue-id>]"
	issueDeleteUsage              = "Usage: az ticket delete [--project <project-id>] --confirm [--id <issue-id>] [--json] [<issue-id>]"
	issueUnarchiveUsage           = "Usage: az ticket unarchive [--project <project-id>] [--id <issue-id>] [--json] [--with-parents] [--cascade-children] [<issue-id>]"
	issueImageAddUsage            = "Usage: az ticket image add [--project <project-id>] [--issue-id <issue-id>] [--path <file>] [<issue-id> <file>] [--json]"
	issueImageRemoveUsage         = "Usage: az ticket image remove [--project <project-id>] [--issue-id <issue-id>] [--attachment-id <attachment-id>] [<issue-id> <attachment-id>] [--json]"
	issueDocumentAddUsage         = "Usage: az ticket document add [--project <project-id>] [--issue-id <issue-id>] [--path <file>] [<issue-id> <file>] [--json]"
	issueDocumentListUsage        = "Usage: az ticket document list [--project <project-id>] [--issue-id <issue-id>] [<issue-id>] [--json]"
	issueDocumentRemoveUsage      = "Usage: az ticket document remove [--project <project-id>] [--issue-id <issue-id>] [--attachment-id <attachment-id>] [<issue-id> <attachment-id>] [--json]"
	issueDepAddUsage              = "Usage: az ticket dep add [--project <project-id>] [--issue-id <issue-id>] [--depends-on-id <depends-on-id>] [<issue-id> <depends-on-id>] [--type blocks|related|parent-child|discovered-from|created-in] [--force-parent-change] [--json]"
	issueDepRemoveUsage           = "Usage: az ticket dep remove [--project <project-id>] [--issue-id <issue-id>] [--depends-on-id <depends-on-id>] [<issue-id> <depends-on-id>] [--type blocks|related|parent-child|discovered-from|created-in] [--confirm] [--confirm-parent-orphan] [--json]"
	...
	issueFanoutReadyUsage         = "Usage: az ticket fanout ready [--project <project-id>] --root <issue-id> [--json]"
	issueFanoutDriftUsage         = "Usage: az ticket fanout drift [--project <project-id>] --issue <issue-id> [--worktree <path>] [--json] [--fail-on-out]"
