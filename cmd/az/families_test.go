...
func TestRunSpecCommandHelpAndValidation(t *testing.T) {
	...
	if !strings.Contains(helpOut, "Usage: az spec <req|link|read|pack|graph|slice|lint|parity> [arguments]") {
		t.Fatalf("help output = %q", helpOut)
	}
	...
	if !strings.Contains(helpOut, "az spec slice gate --slice <slice-id>") {
		t.Fatalf("help output missing slice gate grammar = %q", helpOut)
	}
