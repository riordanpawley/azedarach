...
export const createSessionHandlers = (ctx: HandlerContext) => ({
	 ...
	 * Uses a lighter model for faster, cheaper conversations.
	 */
	chatAboutTask: () =>
		Effect.gen(function* () {
			const task = yield* ctx.getSelectedTask()
			...
			if (task.sessionState !== "idle") {
				yield* ctx.toast.show("error", `Cannot start chat: task is ${task.sessionState}`)
				return
			}

			yield* ctx.sessionManager
				.start({
					beadId: task.id,
					projectPath: process.cwd(),
					model: "haiku",
				...
				.pipe(
					...
					Effect.catchAll(ctx.showErrorToast("Failed to start chat")),
				)
		}),
