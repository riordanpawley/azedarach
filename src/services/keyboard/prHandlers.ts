...
import type { MergeConflictError } from "../../core/PRWorkflow"
				...
				yield* ctx.prWorkflow.mergeToMain({ beadId, projectPath: process.cwd() }).pipe(
					...
					Effect.catchAll((error: MergeConflictError | { _tag?: string; message?: string }) => {
						if (error._tag === "MergeConflictError") {
							return ctx.toast.show("error", `Merge conflict: ${error.message}`)
						}
						const msg =
							error && typeof error === "object" && "message" in error
								? String(error.message)
								: String(error)
						return ctx.toast.show("error", `Merge failed: ${msg}`)
					}),
				)
			}),
		)
