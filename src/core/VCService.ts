...
import { Effect, Context, Layer, Data, Ref, Option } from "effect"
import { Command, CommandExecutor } from "@effect/platform"
import { BunContext } from "@effect/platform-bun"
import { TmuxService, TmuxServiceLive, TmuxError } from "./TmuxService.js"
...
export interface VCServiceImpl {
   ...
   * ```
   */
  readonly stopAutoPilot: () => Effect.Effect<void, VCError | TmuxError, CommandExecutor.CommandExecutor>
   ...
   * ```
   */
  readonly getAttachCommand: () => Effect.Effect<string, VCNotRunningError, CommandExecutor.CommandExecutor>
  ...
  return VCService.of({
    ...
    stopAutoPilot: () =>
      Effect.gen(function* () {
        ...
        yield* tmux.sendKeys(VC_SESSION_NAME, "/exit").pipe(
        ...
        if (stillRunning) {
          yield* tmux.killSession(VC_SESSION_NAME).pipe(
            Effect.catchAll(() => Effect.void)
          )
        }
    ...
    isAutoPilotRunning: () =>
      Effect.gen(function* () {
        const hasSession = yield* tmux.hasSession(VC_SESSION_NAME).pipe(
          Effect.catchAll(() => Effect.succeed(false))
        )
    ...
    sendCommand: (command) =>
      Effect.gen(function* () {
        ...
        yield* tmux.sendKeys(VC_SESSION_NAME, command).pipe(
          Effect.catchTag("SessionNotFoundError", () =>
            Effect.fail(new VCNotRunningError({ message: "VC session not found" }))
          )
        )
        yield* tmux.sendKeys(VC_SESSION_NAME, "Enter").pipe(
          Effect.catchTag("SessionNotFoundError", () =>
            Effect.fail(new VCNotRunningError({ message: "VC session not found" }))
          )
        )
    ...
    toggleAutoPilot: () =>
      Effect.gen(function* () {
        ...
        const running = yield* tmux.hasSession(VC_SESSION_NAME).pipe(
          Effect.catchAll(() => Effect.succeed(false))
        )
