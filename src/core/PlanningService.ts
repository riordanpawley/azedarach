...
export interface PlanningState {
	readonly status: "idle" | "generating" | "reviewing" | "refining" | "creating_beads" | "complete" | "error"
...
export class PlanningService extends Effect.Service<PlanningService>()("PlanningService", {
	...
	effect: Effect.gen(function* () {
		 ...
		 */
		const generatePlan = (featureDescription: string): Effect.Effect<Plan, PlanningError | AIResponseError> =>
			Effect.gen(function* () {
		 ...
		 */
		const reviewPlan = (plan: Plan): Effect.Effect<ReviewFeedback, PlanningError | AIResponseError> =>
			Effect.gen(function* () {
		...
		const refinePlan = (
			...
			Effect.gen(function* () {
				...
				const prompt = REFINEMENT_PROMPT.replace("{FEEDBACK}", JSON.stringify(feedback, null, 2)).replace(
					"{PLAN}",
		 ...
		 */
		const createBeadsFromPlan = (
			plan: Plan,
		): Effect.Effect<ReadonlyArray<Issue>, PlanningError> =>
			Effect.gen(function* () {
