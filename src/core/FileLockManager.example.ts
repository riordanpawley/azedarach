...
const example5_WaitQueue = Effect.gen(function* () {
	...
	if (state) {
		yield* Console.log(
			`  → Lock state: exclusive=${state.exclusiveHolder}, waiting=${state.waitingCount}`,
		)
	}
...
const example6_LockStateInspection = Effect.gen(function* () {
	...
	yield* Console.log(
		`  → Final state: ${finalState === null ? "no locks (cleaned up)" : "has locks"}\n`,
	)
})
...
const example7_InterruptionCleanup = Effect.gen(function* () {
	...
	yield* Console.log(
		`  → Lock state after interrupt: ${afterState === null ? "released" : "still held"}\n`,
	)
})
...
// Main program
const program = runAllExamples.pipe(Effect.provide(FileLockManagerLive))
