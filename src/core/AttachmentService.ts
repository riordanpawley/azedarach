...
import { TerminalService, NotInsideTmuxError, TmuxCommandError } from "./TerminalService"
...
export interface AttachmentServiceI {
   ...
   * ```
   */
  readonly attachExternal: (sessionId: string) => Effect.Effect<void, AttachmentError | SessionNotFoundError>
   ...
   * ```
   */
  readonly attachInline: (sessionId: string) => Effect.Effect<void, AttachmentError | SessionNotFoundError>
    ...
    attachInline: (sessionId: string) =>
      Effect.gen(function* () {
        ...
        return yield* Effect.fail(
          new AttachmentError({
            message: "Inline attachment not yet implemented. Use external attachment (key 'a') instead.",
            sessionId,
          })
        )
      }),
 ...
 */
export const hasAttached = (
  sessionId: string
): Effect.Effect<boolean, never, AttachmentService> =>
