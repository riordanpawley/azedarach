...
export interface EditorServiceImpl {
  ...
  readonly editBead: (
    bead: Issue
  ) => Effect.Effect<void, ParseMarkdownError | EditorError, CommandExecutor.CommandExecutor | BeadsClient | FileSystem.FileSystem>
}
...
const serializeBeadToMarkdown = (bead: Issue): string => {
  ...
  // Metadata section
  lines.push(`Type:     ${typeToLabel(bead.issue_type)}        (read-only - changing requires delete+create)`)
      ...
      for (const line of metadataLines) {
        ...
        if (line.startsWith("Labels:")) {
          ...
          const parsedLabels = labelsValue
            ? labelsValue.split(",").map((l) => l.trim()).filter(Boolean)
            : []
