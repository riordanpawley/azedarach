...
7. Example matrix:
   ...
   - `session.agent_input_delivery` automated agent text delivery -> `hybrid` (refresh durable managed-agent identity and hook-backed readiness, then compare with exact live tmux pane/PID, attachment state, and a tool-specific empty-composer observation under a per-pane lease; attached or ambiguous targets fail closed)
