...
7. Example matrix:
   ...
   - `session.agent_input_delivery` automated agent text delivery -> `hybrid` (persist an incarnation-bound intent plus a session-and-incarnation-scoped cross-daemon exclusion lease first, refresh durable managed-agent identity and hook-backed readiness, then require an explicitly enabled Codex app-server capability and direct exact-thread submission while a daemon-managed tmux gate preserves attached-client flags, makes managed clients read-only, fences new attaches/switches, and revalidates pane/PID plus hook-bound thread identity immediately before `turn/start`; tmux capture/send/paste is never acknowledgement, generic RPC or transport uncertainty remains durably ambiguous without automatic retry, only a typed pre-acceptance rejection may requeue, and unavailable gate/app-server proof remains durably queued)
