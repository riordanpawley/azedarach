...
7. Example matrix:
   ...
   - `session.agent_input_delivery` automated agent text delivery -> `hybrid` (persist an incarnation-bound intent and cross-daemon lease first, refresh durable managed-agent identity and hook-backed readiness, then require a native receiver to atomically prove its tool-owned composer empty, exclude human input through submission, and return an exact intent/incarnation acknowledgement; tmux capture/send/paste is never acknowledgement and unavailable native proof remains durably queued)
