...
7. Example matrix:
   ...
   - `session.agent_input_delivery` automated agent text delivery -> `hybrid` (persist an incarnation-bound intent and cross-daemon lease first, refresh durable managed-agent identity and hook-backed readiness, then require direct exact-thread app-server submission while a daemon-managed tmux gate preserves attached-client flags, makes managed clients read-only, fences new attaches/switches, and revalidates pane/PID/thread identity immediately before `turn/start`; tmux capture/send/paste is never acknowledgement, transport-uncertain submissions remain durably ambiguous without automatic retry, and unavailable gate/app-server proof remains durably queued)
