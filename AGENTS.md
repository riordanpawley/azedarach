...
7. Example matrix:
   ...
   - daemon-owned sequential publication queue -> `hybrid` (refresh the durable accepted-patch operation before each transition, then build and validate an isolated synthetic Git candidate; serialize configured-base updates and compare source/base/candidate identities before exact apply and terminal close)
