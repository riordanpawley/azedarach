...
7. Example matrix:
   ...
   - `session.agent_input_delivery` automated agent text delivery -> `hybrid` (persist an incarnation-bound intent plus a session-scoped cross-daemon exclusion lease whose incarnation and token are fenced values, refresh durable managed-agent identity and hook-backed readiness, then require an explicitly enabled Codex app-server capability and direct exact-thread submission while a daemon-managed tmux gate preserves attached-client flags, makes managed clients read-only, fences new attaches/switches, and synchronously renews/revalidates the exact durable fence plus pane/PID/hook-bound thread immediately before a lease-deadline-bounded `turn/start`; actionable orchestrator mail is first committed to the project-scoped monotonic observation stream, then materialized as a project/issue/session/incarnation-bound wake keyed by the observation ID, with busy workers deferred to hook-backed readiness and restart reconciliation recovering undelivered wakes exactly once; expiry is parsed and compared exactly inside the store transaction, persisted gates carry project/session/incarnation/owner/token identity, owner-aware renew/release/begin calls fence old daemons, and takeover/recovery first transfers ownership without rotating the marker's incarnation/token, restores and removes that exact gate, then CAS-rotates to the new delivery fence so every crash boundary retains durable recovery authority; tmux capture/send/paste is never acknowledgement, every `turn/start` RPC or transport error remains durably ambiguous without automatic retry, and unavailable pre-submission gate/app-server proof remains durably queued with state counts exposed by runtime diagnostics)
   - `session.managed_agent_identity` exact managed-pane and incarnation gate -> `hybrid` (refresh the durable logical-pane, tmux-pane, pane-PID, daemon launch-incarnation, and hook-reported native thread binding, then require an exact live tmux/process match; reused pane IDs, stale hooks, picker/setup UI, and missing native thread identity fail closed)
   - `session.managed_agent_restart` exact supervised agent replacement -> `hybrid` (refresh durable session role/scope, exact orchestrator lease authority when applicable, and managed-agent identity; require the role/scope/lease to agree with the canonical live tmux session and exact pane/PID before replacement; serialize orchestrators through their exact scope lease; respawn only that pane through the canonical launch artifact in the authoritative project root or issue worktree; then require a distinct hook-acknowledged incarnation and live pane/PID before reporting success or recovering without a second respawn)
   ...
   - `task.split_intent` idempotent child creation and isolated-session launch -> `hybrid` (atomically bind the project-scoped client intent and normalized request to one child plus parent-child/created-in edges in the durable issue projection, then reuse the exact orchestration start intent and operation against live session/worktree runtime; conflicting intent reuse fails closed)
   - `orchestration.project_loop` durable watch cursor, deterministic action replay, non-blocking start/review scheduling, and project-scope quiescent continuation -> `projection` (durable issue observation stream + orchestration checkpoint refreshed before each loop decision; daemon-owned starts continue without a model turn, while exact review epochs produce scope-bound semantic wake keys and queued review stays visible without globally suppressing unrelated starts)
  ...
  - `task.read_committed_revision` same-daemon mutation/read floor and cross-daemon lifecycle convergence -> `projection` (authoritative issue commits advance or invalidate the disposable task materializer before acknowledgement; list/search/detail/get-many and close-preflight fence against durable keyed deltas before reading, then synchronously refresh their bounded runtime facts with strict error propagation, and committed session cleanup cannot acknowledge a fresh task view when that refresh fails; only bootstrap/background enrichment may explicitly degrade to ticket-only data)
   ...
   - rooted orchestrator bootstrap delivery -> `hybrid` (one physical top-level session has one mutually exclusive desired role/scope; epic `session.start` and rooted ticket attach/stop delegate to exact rooted-orchestrator authority; every rooted transition atomically retires matching legacy worker intent; refreshed durable accepted prompt acknowledgement binds the exact session, pane, PID, launch incarnation, native agent thread, prompt hash, and live tmux runtime marker; restart replacement is serialized by exact rooted scope and must preserve rooted intent, resume the exact thread without interactive picker/setup UI, re-deliver only after hook readiness, and persist acknowledgement before success)
   - rooted orchestration dependency admission -> `projection` (wait for the daemon-local issue/dependency materializer to reach the durable delivery head, resolve the requested root's ancestor containment chain, then reject local or inherited unresolved root blockers before lease, intent, lifecycle, worktree, or tmux mutation; the same ancestor-aware gate suppresses descendant fanout readiness)
   - rooted parent orchestration continuation -> `hybrid` (durable rooted lease/cursor + refreshed direct runnable, review, nested-root, scoped worker-blocker, interaction, completion, and session projections + managed-agent identity/activity; exact durable blocker-event identities are actionable while progress-only observations are not; actionable semantic revisions enqueue one scope-bound, session/incarnation-bound durable agent-input intent, action/idle transitions advance a durable generation so recurring actions and replacement targets remain deliverable, busy agents defer delivery, equivalent state coalesces, and restart retries unacknowledged intent exactly once)
   ...
   - layered publication evidence -> `projection` (immutable patch-review, active-path, and exact synthetic merge-result evidence with append-only invalidation provenance; unrelated base movement retains patch evidence, while source/decision/dirty/conflict/overlap/policy/environment and configured high-risk uncertainty invalidate selectively and explainably)
   - `task.publication_queue` daemon-owned sequential publication queue -> `hybrid` (refresh and atomically claim the durable accepted-patch operation with a unique expiring attempt token before execution; require the canonical typed reviewer owner ID, exact orchestrator owner kind, review epoch, accepted-review event, publication-operation binding, and live attempt token through validation, retry, exact apply, lease consumption, and terminal close; reclaim only expired attempts, build and validate an isolated synthetic Git candidate, serialize configured-base updates, and compare source/base/candidate identities before exact apply)
...
- Personal provider credentials live only in the SOPS-encrypted `config/env/development.env`; `.envrc` decrypts it directly into the shell after loading the Nix environment. Edit it with `sops`, keep recipients in `.sops.yaml`, and reserve `.envrc.local` for unshared machine-specific overrides.
