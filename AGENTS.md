...
7. Example matrix:
   ...
   - `task.read_committed_revision` same-daemon mutation/read floor and cross-daemon lifecycle convergence -> `projection` (authoritative issue commit advances or invalidates the disposable task materializer before acknowledgement; durable keyed delta replay advances other daemons, and enrichment failure cannot roll lifecycle fields back)
