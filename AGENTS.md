...
7. Example matrix:
   ...
   - `session.managed_agent_restart` exact supervised agent replacement -> `hybrid` (refresh the durable managed-agent identity, require an exact live tmux pane/PID match, respawn only that pane through the canonical launch artifact, then require a distinct hook-acknowledged incarnation and live pane/PID before reporting success)
