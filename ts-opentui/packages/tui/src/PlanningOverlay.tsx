...
import type { Plan, PlanningState, ReviewFeedback } from "../../../src/core/PlanningService.js"
...
const ReviewSummary = ({ feedback }: { feedback: ReviewFeedback }) => (
	<box flexDirection="column" marginTop={1}>
		<box flexDirection="row">
			...
			<text fg={feedback.score > 80 ? theme.green : feedback.score > 50 ? theme.yellow : theme.red}>
				{`${feedback.score}/100`}
			</text>
		...
		{feedback.issues.length > 0 && (
			<box flexDirection="column" marginTop={1}>
				...
				{feedback.issues.slice(0, 3).map((issue) => (
					<text key={issue} fg={theme.subtext0} paddingLeft={2}>
						{`• ${issue.slice(0, 60)}`}
					</text>
				))}
			</box>
 ...
 */
const ProgressPhase = ({ state, onCancel }: { state: PlanningState; onCancel: () => void }) => {
	useKeyboard((event) => {
		if (event.name === "escape") {
			onCancel()
		}
	...
	const latestFeedback =
		state.reviewHistory.length > 0 ? state.reviewHistory[state.reviewHistory.length - 1] : null
