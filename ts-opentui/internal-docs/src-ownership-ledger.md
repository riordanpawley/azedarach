...
```
ts-opentui/packages/entry/src/index.ts:17:import { KeyboardService as LegacyKeyboardService } from "../../../src/runtime/appServicesFacade.js"
```
...
- 2026-03-20 `abj` replaced TUI transitive sqlite ownership with package-local `packages/tui/src/services/TuiProjectContextService.ts`, `packages/tui/src/services/ProjectStateService.ts`, and `packages/tui/src/services/projectUiState.ts`; removed `ProjectService` and `ProjectStateService` from `packages/tui/src/utils/legacyRuntimeServices.ts`; and kept the remaining direct package->`src` seam to keyboard only.
- 2026-03-20 `abk` moved the TUI keyboard contract to `packages/tui/src/services/KeyboardService.ts`, deleted `packages/tui/src/utils/legacyRuntimeServices.ts`, and shifted legacy keyboard live composition to `packages/entry/src/index.ts` via `src/runtime/appServicesFacade.ts`.
...
- Current remaining runtime-facade package edge is entry-only through `packages/entry/src/index.ts`.
