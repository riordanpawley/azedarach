...
describe("Thin client boundary (daemon rpc authority)", () => {
	...
	it("keeps daemon-overlap authority services out of daemon-rpc runtime composition", async () => {
		...
		expect(source).not.toContain("sessionManagerBoundaryLayer")
		expect(source).not.toContain("prWorkflowBoundaryLayer")
		expect(source).not.toContain("issueEditorBoundaryLayer")
	})
})
