...
export const isActiveSessionState = (state: SessionState): boolean =>
	ACTIVE_SESSION_STATES.has(state)
...
export class SessionManager extends Effect.Service<SessionManager>()("SessionManager", {
	...
	effect: Effect.gen(function* () {
		...
		const startsInProgressRef = yield* Ref.make<HashMap.HashMap<string, true>>(HashMap.empty())
		...
		const canonicalizeSessionMap = (
			...
			for (const [storedIssueId, session] of HashMap.entries(sessions)) {
				...
				const canonicalSession =
					session.issueId === canonicalIssueId ? session : { ...session, issueId: canonicalIssueId }
		...
		const getTmuxSessionOption = (sessionName: string, optionName: string) =>
			Command.string(Command.make("tmux", "show-option", "-t", sessionName, "-v", optionName)).pipe(
		...
		const acquireStartLock = (issueId: string) =>
			Ref.modify(startsInProgressRef, (startsInProgress) => {
				if (HashMap.has(startsInProgress, issueId)) {
					return [false, startsInProgress] as const
				}
				return [true, HashMap.set(startsInProgress, issueId, true)] as const
			})

		const releaseStartLock = (issueId: string) =>
			Ref.update(startsInProgressRef, (startsInProgress) =>
				HashMap.remove(startsInProgress, issueId),
			)
						...
						(hasStartLock) =>
							hasStartLock
								? Effect.gen(function* () {
										...
										if (!effectiveBaseBranch) {
											...
											if (parentEpic) {
												...
												yield* Effect.log(
													`Child task ${issueId} will branch from epic ${parentEpic.id}`,
												)
											}
										}
													...
													? Effect.gen(function* () {
															...
															if (acquired.createdNewSession) {
																yield* tmuxService.killSession(tmuxSessionName).pipe(
																	Effect.tap(() =>
																		Effect.logWarning(
																			`Rolled back tmux session ${tmuxSessionName}`,
																		),
																	),
																	Effect.catchAll((error) =>
																		Effect.logWarning(
								...
								: Effect.gen(function* () {
										...
										return yield* Effect.fail(
											new SessionLimitError({
												message: `Session start already in progress for ${issueId}`,
												limit: 0,
												current: 0,
											}),
										)
									}),
						(hasStartLock) => (hasStartLock ? releaseStartLock(issueId) : Effect.void),
					)
				}),
