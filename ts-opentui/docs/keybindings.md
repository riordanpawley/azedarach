...
`$schema` points to a companion `.azedarach.schema.json` file generated by Azedarach when settings are saved or validated.
...
When cleaning up multiple worktrees and branches (`Space` `d` with selections), a choice dialog appears:

| Key | Action | Description |
|-----|--------|-------------|
| `w` | Worktrees + branches | Delete worktrees/branches but keep tracker open |
...
4. Press `Space` `d` → `w` to cleanup worktrees + branches
```
