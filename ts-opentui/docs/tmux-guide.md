...
├────────────────────────────────────────────────────────┤
│ Session Commands:                                       │
│   Ctrl-a g     Return to az TUI (default Az binding)   │
...
5. **Press `Ctrl-a g` from any Claude session to return to az**
...
Great for quick `git status`, `az issue list --output json --compact --all`, or any one-off command!
...
# From Claude session: Ctrl-a g to return to az TUI
```

This two-keystroke pattern makes az the hub for all session navigation:
- `Space` `a` → go TO a Claude session (from az)
- `Ctrl-a g` → go BACK to az (from anywhere)

Note: `Ctrl-a Ctrl-a` keeps native tmux `send-prefix` behavior.
...
az issue list --output json --compact --all | head -1
