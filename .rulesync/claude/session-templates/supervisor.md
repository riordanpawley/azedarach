...
│                    SUPERVISOR LOOP                          │
├─────────────────────────────────────────────────────────────┤
│  1. DISCOVER    → Find ready tasks (az issue --help)               │
...
```bash
az issue update [TASK_ID] --notes="ASSESSMENT:
...
| WAITING | Agent needs input | Check issue notes, may need intervention |
| IDLE | Session ended | Proceed to ANALYZE |
| (no session) | Session killed/crashed | Check issue status, may need retry |
...
```bash
az issue get [TASK_ID]  # See notes for progress updates
```
...
```bash
az issue update [TASK_ID] --notes="ANALYSIS:
...
### Close (Success)
```bash
az issue close [TASK_ID] --reason="Completed: [summary]. Quality gates passed."
```

### Block (Failure)
```bash
az issue update [TASK_ID] --status=blocked --notes="BLOCKED:
- Reason: [Type-check failed / Tests failed / etc]
- Errors: [Key error messages]
- Next: [What needs to happen to unblock]"
az issue update [TASK_ID] --assignee=""  # Release claim
```

### Retry (Transient failure)
```bash
az kill [TASK_ID]  # Kill stuck session
az issue update [TASK_ID] --status=open --assignee=""  # Reset
...
READY_COUNT=$(az issue --help | wc -l)
...
### Discovery
```bash
az issue --help                          # Find unblocked tasks
az issue --help     # Tasks in this epic
az issue get [ID]                      # Task details
```
...
### Task Management
```bash
az issue update [ID] --status=X         # Update status
az issue update [ID] --notes="..."      # Add notes
az issue update [ID] --assignee=""      # Release claim
az issue close [ID] --reason="..."      # Mark complete
az issue create --title="..." --type=X  # Create discovered issue
az issue update [A] --parent [B]       # Link issues
```
...
### Session Crash
```bash
# Session disappeared without updating issue
az status | grep [TASK_ID] || {
  echo "Session crashed"
  az issue update [TASK_ID] --status=open --assignee=""
  az issue update [TASK_ID] --notes="Session crashed, needs retry"
}
```
...
az issue get [TASK_ID]  # Check notes for what it needs
...
```bash
# Type-check failed
az issue update [TASK_ID] --status=blocked
az issue update [TASK_ID] --notes="BLOCKED: Type-check failed
...
1. **Discover** ready tasks: `az issue --help`
