...
```bash
az issue update [TASK_ID] --status=in_progress --assignee="[UNIQUE_SESSION_ID]"
```
...
## Design Notes
[Design notes from issue, if any]
...
7. Close the task: az issue close [TASK_ID] --reason="[summary]"

## Progress Notes Format
Update notes at key milestones using:
```bash
az issue update [TASK_ID] --notes="COMPLETED:
- [What's done]
...
- If you discover issues outside scope, create a new issue and link it
```
...
3. Claims the issue with `--assignee=[session-name]`
...
az issue get az-001  # See issue notes for progress
```
...
**Check issue notes for progress:**
```bash
az issue get [TASK_ID]  # See notes field for progress updates
```

### 5. Handle Completion

When a subagent completes successfully:
1. Verify the task was closed: `az issue get [TASK_ID]`
...
3. Update issue status: `az issue update [TASK_ID] --status=blocked --notes="[reason]"`
4. Clear assignee if abandoning: `az issue update [TASK_ID] --assignee=""`

### 6. Complete the Epic

When all child tasks are closed:
1. Verify all work: `az issue get {{EPIC_ID}}`
2. Run final type-check: `bun run type-check`
3. Update epic notes with summary
4. Close the epic: `az issue close {{EPIC_ID}} --reason="All child tasks completed"`
...
| Action | Command |
|--------|---------|
| Show task details | `az issue get [ID]` |
| Claim task | `az issue update [ID] --status=in_progress --assignee="[SESSION]"` |
| Update progress notes | `az issue update [ID] --notes="..."` |
| Release claim | `az issue update [ID] --assignee=""` |
| Close completed task | `az issue close [ID] --reason="..."` |
| Create discovered issue | `az issue create --title="Found: ..." --type=bug` |
| Link discovered issue | `az issue update [NEW_ID] --parent [PARENT_ID]` |
| Check blocked tasks | `az issue --help` |
| Check ready tasks | `az issue --help` |
