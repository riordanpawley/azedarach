...
2. **Report discoveries** - If you find bugs, improvements, or blockers outside your scope, create a new issue:
   ```bash
   az issue create --title="Found: <issue>" --type=bug
   az issue update <new-id> --parent {{TASK_ID}}
   ```

3. **Update status** - Keep your issue updated with progress:
   ```bash
   az issue update {{TASK_ID}} --notes="<progress notes>"
   ```

4. **Signal completion** - When done, close your task:
   ```bash
   az issue close {{TASK_ID}} --reason="<what was accomplished>"
   ```
