...
- reason: "{{TASK_ID}}"    # Link to issue for tracking
```
...
4. Claim issue:
   - az issue update {{TASK_ID}} --status=in_progress --assignee="{{TASK_ID}}-worker"
...
6. If discover new work:
   - Create issue: az issue create --title="Found: ..." --type=bug
   - Link it: az issue update NEW_ID --parent {{TASK_ID}}
...
2. Update issue notes with state
...
│              SUPERVISOR LOOP (with Agent Mail)              │
├─────────────────────────────────────────────────────────────┤
│  1. DISCOVER    → az issue --help + check inbox for discoveries    │
