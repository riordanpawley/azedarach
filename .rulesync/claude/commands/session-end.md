...
**Case A: Work matches existing in-progress issue**
- Verify issue has recent notes (check with `az issue get <id>`)
- If notes missing or stale, UPDATE issue with missing context

**Case B: Work NOT tracked in any issue**
- Create new issue for untracked work
- Include comprehensive notes about what was done

**Case C: Work completed but issue still open**
...
For each in-progress issue, check if notes are comprehensive:

```bash
az issue update AZ-123 --notes="
...
To Resume Work:
   1. Check ready work: az issue --help
   2. Review retrospective: /retro-review
   3. Continue on: [most likely next issue based on notes]
