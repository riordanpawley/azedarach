...
For each selected issue, create a issue:

```bash
az issue create --title="Extracted title from suggested action" \
  --type=task \
  --priority=2 \
  --description="
...
Next steps:
- Review issues: az issue --help
- Work on tasks: az issue --help
...
- Link back to session file in issue description for full context
