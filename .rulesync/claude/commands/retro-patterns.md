...
- **Status** (was issue created? check for REVIEWED marker with issue IDs)
...
For each selected pattern, create a comprehensive issue:

```bash
az issue create --title="Address recurring pattern: [Pattern Name]" \
  --type=task \
  --priority=1 \
  --description="
...
Next steps:
- Work on recurring pattern tasks: az issue --help
