...
For `ts-opentui` behavior changes, update `az spec` requirements/links in the same task; if there is no spec delta, log `Spec impact: none` with file-specific rationale in issue notes before completion.
