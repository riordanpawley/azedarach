...
4. **Branch Workflow**: Azedarach pushes branches at worktree creation (`git push -u`) so they have upstreams and use normal git pull/push flow. When already in the target worktree, use plain `git` commands; use `git -C <path>` only when intentionally targeting a different path.
