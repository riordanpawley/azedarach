...
- `session.agent_input_delivery`: `hybrid` (refreshed durable managed-agent identity and hook-backed readiness + exact live tmux pane/PID, attachment state, and tool-specific empty-composer proof under a per-pane lease; ambiguity fails closed without writing).
