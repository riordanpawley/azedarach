...
- `task.publication_queue`: `hybrid` (refresh the durable accepted-patch queue operation first, then create and validate an isolated synthetic Git candidate and compare-and-apply only while the pinned source and base identities remain exact).
