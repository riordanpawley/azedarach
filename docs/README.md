...
- `orchestration.parent_continuation`: `hybrid` (durable rooted lease/cursor + refreshed direct runnable, review, nested-root, interaction, completion, and session projections compared with live tmux and managed-agent identity/activity; actionable semantic revisions enqueue one scope-bound durable input intent, action/idle transitions advance a durable generation so recurring actions remain deliverable, busy agents defer delivery, equivalent state coalesces, and restart retries unacknowledged intent exactly once).
