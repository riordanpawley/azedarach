...
| Slow consumer drops task mutations under telemetry pressure | `TestPriorityTaskMutationEvictsQueuedRuntimeTelemetry`, `TestPriorityTaskMutationDoesNotEvictQueuedTaskMutations` | `go test ./internal/daemon/publish -run 'TestPriorityTaskMutationEvictsQueuedRuntimeTelemetry|TestPriorityTaskMutationDoesNotEvictQueuedTaskMutations'` | A saturated subscriber queue evicts low-priority runtime telemetry for task mutations, but never evicts an already queued task mutation. |
...
go test ./internal/daemon/publish -run 'TestPriorityTaskMutationEvictsQueuedRuntimeTelemetry|TestPriorityTaskMutationDoesNotEvictQueuedTaskMutations'
