...
func TestRenderPrimeOrchestrationSectionExplainsRuntimeContinuationGuard(t *testing.T) {
	...
	section := renderPrimeOrchestrationSection(protocol.OrchestrationSnapshot{
		...
		ValidationCapacity:   &domain.ValidationSnapshot{Revision: 4, Freshness: domain.ValidationSnapshotFresh, Active: []domain.ValidationRequest{{RequestID: "gate"}}, Queued: []domain.ValidationRequest{{RequestID: "waiter"}}},
	})
	for _, want := range []string{"Runtime persistence guard: wake-required", "direct nested root active", "consume the durable cursor and continue", "cursor=17", "Validation capacity: freshness=fresh active=1 queued=1 revision=4", "inspect with `az validation status`"} {
