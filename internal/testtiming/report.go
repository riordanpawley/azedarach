...
func WriteMarkdown(w io.Writer, m Measurement) error {
	...
	write("- Validation lease: held `%t`; request `%s`; class `%s`; scope `%s`; purpose `%s`; execution `%s`; source `%s`; override `%s`; profile `%s`\n", m.ValidationLease.Held, m.ValidationLease.RequestID, m.ValidationLease.Class, m.ValidationLease.Scope, m.ValidationLease.Purpose, m.ValidationLease.Execution, m.ValidationLease.AuthoritativeRequestID, m.ValidationLease.Override, m.ValidationLease.Profile)
	write("- Database-clone isolation: mode `%s`; configured `%t`; package identities `%d`\n", m.CloneIsolation.Mode, m.CloneIsolation.Configured, len(m.CloneIsolation.Packages))
	for _, identity := range m.CloneIsolation.Packages {
		write("  - `%s`: user `%s`; projects `%s`\n", identity.Package, identity.UserDB, strings.Join(identity.ProjectDBs, ", "))
	}
