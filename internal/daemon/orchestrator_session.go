	...
	case protocol.CommandOrchestratorSessionStart:
		...
		if !live {
			...
			if body.Scope.Kind == domain.OrchestrationScopeRooted {
				...
				if launchErr := d.tmux.NewSessionWithCommandAndEnvironment(ctx, acquired.Lease.SessionID, workdir, artifact.Command, daemonScopeTmuxEnvironment()); launchErr != nil {
					appeared, _ := d.tmux.HasSession(ctx, acquired.Lease.SessionID)
					if !appeared {
						artifact.remove()
