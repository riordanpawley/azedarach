...
func TestPublishGitStatusProjectionEventIncludesRuntimeDelta(t *testing.T) {
	...
	if body.Runtime.Projection.Git.GitAdditions != 2 || body.Runtime.Projection.Git.GitDeletions != 1 {
		t.Fatalf("runtime git stats = %+v, want additions/deletions 2/1", body.Runtime.Projection.Git)
	}
