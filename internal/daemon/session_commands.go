...
func buildOrchestratorRolePrompt(base string) string {
	...
	base += "\nReview revision contract: use each review queue packet's exact `diff_base_revision`, `head_revision`, and `diff_scope` when delegating inspection. The first pass reviews that full scope; later intra-candidate passes inspect only the previous reviewed head through current head plus unresolved findings and affected contracts. Fall back to the full scope when the prior checkpoint cannot be verified, is not an ancestor, or its base/scope changed."
