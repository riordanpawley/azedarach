...
--
-- Schema effects:
--   Create agent_input_delivery_intents with the ambiguous submission state,
--   its pending/incarnation indexes, and durable session-scoped delivery
--   leases with fenced incarnation values and an expiry index.
-- Data effects:
--   None. Existing issue, session, decision, and runtime rows are not read,
--   rewritten, backfilled, or deleted; the new intent and lease tables start
--   empty.
-- Validation effects:
...
--   schema_migrations row for 0052_agent_input_delivery with this artifact's
