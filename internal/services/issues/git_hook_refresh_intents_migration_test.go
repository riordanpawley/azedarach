...
func TestGitHookRefreshIntentsMigrationRejectsColumnConstraintAndIndexDrift(t *testing.T) {
	...
	fixtures := []struct {
		...
		{name: "project id quoted literal whitespace", old: "CHECK (trim(project_id) <> '')", new: "CHECK (trim(project_id) <> ' ')"},
		...
		{name: "requested generation weakened with canonical comment", old: "CHECK (requested_generation > 0)", new: "CHECK (1) /* CHECK (requested_generation > 0) */"},
