		...
		return c.withMutationLock(ctx, func(ctx context.Context) error {
			...
			err = tx.QueryRowContext(ctx, `SELECT operation_id,accepted_review_event_id FROM daemon_publication_operations
				WHERE project_id=? AND (coalesce_key=? OR (issue_id=? AND intent_key=?))
				  ...
				  AND LOWER(actor_id)=LOWER(?) AND reviewer_kind=? AND review_epoch_event_id=? AND patch_evidence_id=?
				...
				operation.ActorID, operation.ReviewerKind, operation.ReviewEpochEventID, operation.PatchEvidenceID,
			...
			result, updateErr := tx.ExecContext(ctx, `UPDATE daemon_publication_operations SET accepted_review_event_id=? WHERE operation_id=? AND accepted_review_event_id IN (0,?)`, eventID, publicationOperationID, eventID)
			if updateErr != nil {
				return c.wrapError("append-accepted-review-publication", issueID, updateErr)
			}
			if changed, _ := result.RowsAffected(); changed != 1 {
