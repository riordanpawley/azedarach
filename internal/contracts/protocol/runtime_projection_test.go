...
func TestRuntimeProjectionSnapshotPayloadJSONShapeIsDeterministic(t *testing.T) {
	...
	want := `{"schema_version":2,"protocol_version":13,"snapshot_revision":11,"project_id":"proj-1","projections":[{"project_id":"proj-1","issue_id":"az-42","worktree":{"exists":true,"path":"/tmp/repo-az-42","branch":"riordan/az-42/task","healthy":true,"git_status_updated_at":"2026-04-01T08:05:00.654321Z"},"git":{"has_uncommitted_changes":true,"has_conflicts":true,"conflict_files":["conflicted.go"],"git_additions":7,"git_deletions":3,"git_ahead_count":2,"git_behind_count":1,"active_operation":{"operation_id":"op-123","state":"running","progress_percent":45,"message":"syncing runtime projection"}},"session":{"has_session":true,"session_id":"sess-42","state":"running","total_count":2,"active_count":1,"paused_count":1,"started_at":"2026-04-01T08:00:00.123456Z","updated_at":"2026-04-01T08:05:00.654321Z","worktree":"/tmp/repo-az-42"},"agent":{"status":"working","session_id":"sess-42","updated_at":"2026-04-01T08:05:00.654321Z"}}]}`
	if string(got) != want {
...
func TestRuntimeProjectionZeroValuesAreStable(t *testing.T) {
	...
	got := string(data)
	want := `{"schema_version":2,"protocol_version":13,"snapshot_revision":0,"project_id":"","projections":null}`
	if got != want {
