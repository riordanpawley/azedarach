...
func TestDecodeTaskListSnapshotPayloadRejectsVersionMismatch(t *testing.T) {
	t.Run("schema version", func(t *testing.T) {
		body := []byte(`{"schema_version":99,"protocol_version":10,"snapshot_revision":1,"project_id":"proj","last_checked_at":"2026-04-02T10:31:45Z","freshness":"fresh","tasks":[]}`)
	...
	t.Run("protocol version", func(t *testing.T) {
		...
		if got := err.Error(); !strings.Contains(got, "protocol_version mismatch: expected 10, actual 99") {
			t.Fatalf("error = %q, want expected/actual protocol mismatch", got)
		}
	})

	t.Run("missing freshness metadata", func(t *testing.T) {
		body := []byte(`{"schema_version":2,"protocol_version":10,"snapshot_revision":1,"project_id":"proj","tasks":[]}`)
	...
	t.Run("invalid freshness", func(t *testing.T) {
		body := []byte(`{"schema_version":2,"protocol_version":10,"snapshot_revision":1,"project_id":"proj","last_checked_at":"2026-04-02T10:31:45Z","freshness":"unknown","tasks":[]}`)
