...
func TestProjectionDeltaProtocolVersionClaimAndCompatibility(t *testing.T) {
	...
	if CurrentVersion != 55 {
		t.Fatalf("current protocol version=%d, want 55", CurrentVersion)
	}
	if SupportsProjectionDeltaCommands(47) || !SupportsProjectionDeltaCommands(48) || !SupportsProjectionDeltaCommands(49) || !SupportsProjectionDeltaCommands(50) || !SupportsProjectionDeltaCommands(51) || !SupportsProjectionDeltaCommands(52) || !SupportsProjectionDeltaCommands(53) || !SupportsProjectionDeltaCommands(54) || !SupportsProjectionDeltaCommands(55) || SupportsProjectionDeltaCommands(56) {
		t.Fatal("projection command support window does not span v48-v55")
	}
	if ack := NegotiateHello(Hello{ProtocolVersion: 48}, "daemon-v55"); ack.Accepted || ack.DaemonProtocolVersion != 55 || ack.ErrorCode != ErrorCodeUpgradeRequired {
		t.Fatalf("v48 stale-client handshake=%+v", ack)
	}
	if ack := NegotiateHello(Hello{ProtocolVersion: 54}, "daemon-v55"); ack.Accepted || ack.DaemonProtocolVersion != 55 || ack.ErrorCode != ErrorCodeUpgradeRequired {
		t.Fatalf("v54 stale-generation handshake=%+v", ack)
	}
	if ack := NegotiateHello(Hello{ProtocolVersion: 55}, "daemon-v55"); !ack.Accepted || ack.DaemonProtocolVersion != 55 {
		t.Fatalf("v55 compatibility handshake=%+v", ack)
	}
	if ack := NegotiateHello(Hello{ProtocolVersion: 56}, "daemon-v55"); ack.Accepted || ack.ErrorCode != ErrorCodeIncompatible {
