...
func TestTaskWorkspaceOverlay_View_Golden_SmallViewport(t *testing.T) {
	task := domain.Task{
		ID:          "bal",
		Title:       "add grom renderer for detail view maybe?",
		Status:      domain.StatusOpen,
	...
	overlay := NewTaskWorkspaceOverlay(task, nil, nil, 72, 22)
	got := overlay.View()

	goldenPath := filepath.Join("testdata", "task_workspace_small.golden")
	if os.Getenv("UPDATE_GOLDEN") == "1" {
		if err := os.MkdirAll(filepath.Dir(goldenPath), 0o755); err != nil {
			t.Fatalf("create golden dir: %v", err)
		}
	...
	if got != string(want) {
		t.Fatalf("task workspace small view changed; run UPDATE_GOLDEN=1 go test ./internal/ui/overlay to accept")
	}
}
