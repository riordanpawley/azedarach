...
func TestRender_UsesViewportOnlyOnActiveColumn(t *testing.T) {
	...
	out := normalizeBoardOutput(Render(columns, Cursor{Column: 1, Task: 4}, map[string]bool{}, map[string]RuntimeSignals{}, BuildChildProgress(columnsToTasks(columns)), nil, false, 4, s, width, height))
