...
func TestRender_UsesViewportOnlyOnActiveColumn(t *testing.T) {
	...
	out := normalizeBoardOutput(Render(columns, Cursor{Column: 1, Task: 4}, map[string]bool{}, map[string]RuntimeSignals{}, testChildProgress(columnsToTasks(columns)), nil, false, nil, 4, s, width, height))
