...
func TestGetHints_AllModes(t *testing.T) {
	tests := []struct {
		...
		{types.ModeAction, "h/l: move  s/S/!: start  a: attach  p: pause  R: resume  r: refresh  V: dev  x: stop  u: update  m/b: merge  P/O: PR  M: abort  H: helix  i: attachments  f: diff  w/W: cleanup  e: edit  c: child  5: cancel issue  T/d: tombstone/delete  Esc/q: cancel"},
	}
