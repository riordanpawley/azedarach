...
func TestStatusBar_RenderNormalMode(t *testing.T) {
	...
	if !strings.Contains(result, "O: orchestrator") {
		t.Errorf("Expected status bar to contain orchestrator hint, got: %s", result)
	}
...
func TestGetHints_AllModes(t *testing.T) {
	tests := []struct {
		...
		{types.ModeNormal, "?: help  O: orchestrator  Space: task workspace  g: goto  /: search  f: filter  t: session tree  ,: sort  v: select  Enter: drill  a: attach  c: create  s: settings  r: refresh  n: recover  Q: ops  V: views  G: git  Tab/Shift+Tab: view  p: pull base  q: quit"},
