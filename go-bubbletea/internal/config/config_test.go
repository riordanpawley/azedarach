...
func TestMergeWithDefaults(t *testing.T) {
	...
	assert.Equal(t, ".azedarach", merged.Issues.Path)
}
...
func TestComplexConfig(t *testing.T) {
  ...
  "worktree": {
	...
	assert.Equal(t, ".custom-issues", cfg.Issues.Path)
	assert.Equal(t, 600, cfg.Issues.SyncInterval)
